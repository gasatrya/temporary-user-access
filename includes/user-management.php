 ...
 */
function wp_tua_add_user_registration_fields() {
	// Only show fields for users who can manage expiry settings
	if ( ! wp_tua_current_user_can_manage_expiry() ) {
		return;
	}
	...
	<table class="form-table">
		<?php wp_nonce_field( 'wp_tua_create_user_expiry', 'wp_tua_expiry_nonce' ); ?>
		...
		<tr>
			...
			<td>
				<label for="user_auto_delete">
					...
					echo esc_html( sprintf( __( 'Auto-delete user after expiry (%d days grace period)', 'temporary-user-access' ), WP_TUA_GRACE_PERIOD_DAYS ) );
					?>
				</label>
				<p class="description">
...
add_action( 'user_new_form', 'wp_tua_add_user_registration_fields' );
 ...
 */
function wp_tua_add_user_profile_fields( $user ) {
	// Don't show fields for admin users or if current user can't manage expiry
	if ( wp_tua_is_user_admin( $user->ID ) || ! wp_tua_current_user_can_manage_expiry() ) {
		return;
	}

	$expiry_date = get_user_meta( $user->ID, WP_TUA_USER_EXPIRY_DATE, true );
	$auto_delete = get_user_meta( $user->ID, WP_TUA_USER_AUTO_DELETE, true );
...
add_action( 'show_user_profile', 'wp_tua_add_user_profile_fields' );
add_action( 'edit_user_profile', 'wp_tua_add_user_profile_fields' );
 ...
 * @return WP_Error
 */
function wp_tua_validate_user_fields( $errors, $update, $user ) {
	...
	if ( $update && isset( $user->ID ) ) {
		...
		if ( isset( $_POST['wp_tua_expiry_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['wp_tua_expiry_nonce'] ) ), 'wp_tua_create_user_expiry' ) ) {
			$nonce_verified = true;
		}
	}
	...
	if ( ! empty( $_POST['user_expiry_date'] ) ) {
		...
		if ( ! wp_tua_validate_date( $expiry_date ) ) {
...
add_filter( 'user_profile_update_errors', 'wp_tua_validate_user_fields', 10, 3 );
 ...
 */
function wp_tua_save_user_fields( $user_id ) {
	...
	if ( $user_id > 0 && wp_tua_is_user_admin( $user_id ) ) {
		return;
	}
	...
	if ( isset( $_POST['user_expiry_clear'] ) && '1' === sanitize_text_field( wp_unslash( $_POST['user_expiry_clear'] ) ) ) {
		delete_user_meta( $user_id, WP_TUA_USER_EXPIRY_DATE );
		wp_tua_log( 'User expiry cleared', array( 'user_id' => $user_id ) );
	} elseif ( isset( $_POST['user_expiry_date'] ) ) {
		$expiry_date = sanitize_text_field( wp_unslash( $_POST['user_expiry_date'] ) );
		if ( empty( $expiry_date ) ) {
			delete_user_meta( $user_id, WP_TUA_USER_EXPIRY_DATE );
		} else {
			update_user_meta( $user_id, WP_TUA_USER_EXPIRY_DATE, $expiry_date );
		}
	}
	...
	if ( isset( $_POST['user_auto_delete'] ) ) {
		...
		delete_user_meta( $user_id, WP_TUA_USER_AUTO_DELETE );
	}
}
add_action( 'user_register', 'wp_tua_save_user_fields' );
add_action( 'profile_update', 'wp_tua_save_user_fields' );
 ...
 */
function wp_tua_enqueue_admin_scripts( $hook ) {
...
add_action( 'admin_enqueue_scripts', 'wp_tua_enqueue_admin_scripts' );
 ...
 * @return string
 */
function wp_tua_show_expiry_columns( $value, $column_name, $user_id ) {
...
add_filter( 'manage_users_custom_column', 'wp_tua_show_expiry_columns', 10, 3 );
 ...
 * @return array
 */
function wp_tua_make_expiry_status_sortable( $columns ) {
	$columns['expiry_status'] = 'expiry_status';
	return $columns;
}
add_filter( 'manage_users_sortable_columns', 'wp_tua_make_expiry_status_sortable' );
 ...
 */
function wp_tua_handle_expiry_status_sorting( $query ) {
	...
	$query->query_vars['meta_key'] = WP_TUA_USER_EXPIRY_DATE;
	$query->query_vars['order']    = $order;
}
add_action( 'pre_get_users', 'wp_tua_handle_expiry_status_sorting' );

/**
 * Add CSS for users list table styling
 */
function wp_tua_users_list_styles() {
	echo '<style>
...
add_action( 'admin_head-users.php', 'wp_tua_users_list_styles' );
